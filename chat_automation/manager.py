...
from .cli_common import prepare_webm_transcription_chunks
...
class Conversation:
    ...
    updated_at: str
    url: Optional[str] = (
        None  # ChatGPT conversation URL (e.g., https://chatgpt.com/c/xxx)
    )
...
"""
    ...
    async def _ensure_logged_in(self) -> bool:
        ...
        try:
            login_btn = await chatgpt.page.query_selector(
                '[data-testid="login-button"], button:has-text("Log in")'
            )
    ...
    async def send(self, message: str, wait_for_response: bool = True) -> str:
        ...
        self._current_conversation.messages.append(
            Message(role="user", content=message, timestamp=datetime.now().isoformat())
        )

        try:
            response = await self._chatgpt.chat(
                message, wait_for_response=wait_for_response
            )
    ...
    async def send_voice_file(self, filepath: str, message_prefix: str = "") -> str:
        """Transcribe an audio file through ChatGPT web and send transcript.

        Args:
            filepath: Path to source audio file.
            message_prefix: Optional text prepended before transcript.

        Returns:
            Assistant response after sending transcript.
        """
        temporary_paths: List[str] = []
        try:
            chunks, temporary_paths = prepare_webm_transcription_chunks(
                filepath, max_chunk_seconds=180.0
            )
            ...
            if len(chunks) > 1:
                log(
                    f"Long audio detected; transcribing {len(chunks)} chunks (<= 3 minutes each)"
                )

            transcript_parts: List[str] = []
            for idx, (chunk_path, duration_ms) in enumerate(chunks, start=1):
                if len(chunks) > 1:
                    log(f"Transcribing chunk {idx}/{len(chunks)}")
                chunk_text = await self._chatgpt.transcribe_audio_webm(
                    chunk_path, duration_ms
                )
                if chunk_text.strip():
            ...
            if message_prefix.strip():
                composed = f"{message_prefix.strip()}\n\n{transcript}"

            return await self.send_formatted(composed)
        except Exception as e:
            log(f"Error sending voice file: {e}")
            return f"Error: {str(e)}"
        finally:
    ...
    async def transcribe_voice_file(self, filepath: str) -> str:
        """Transcribe an audio file through ChatGPT web without sending a message."""
        temporary_paths: List[str] = []
        try:
            chunks, temporary_paths = prepare_webm_transcription_chunks(
                filepath, max_chunk_seconds=180.0
            ...
            log(f"Error transcribing voice file: {e}")
            return f"Error: {str(e)}"
        finally:
            for tmp_path in temporary_paths:
                try:
            ...
            "messages": [
                {
                    ...
                    "timestamp_formatted": format_timestamp(m.timestamp),
                }
