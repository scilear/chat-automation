...
""")
    ...
    async def library(self, limit: int = 20):
        ...
        try:
            ...
            for i, conv in enumerate(conversations, 1):
                ...
                type_icon = "[🔍]" if conv.type.value == "search" else "[💬]"
                table.add_row(
                    str(i),
        ...
        try:
            ...
            try:
                ...
                for conv_id in conversation_ids:
                    ...
                    if success:
                        console.print(f"  [green]✓[/green] {conv_id}")
                    else:
                        console.print(f"  [red]✗[/red] {conv_id}")
            finally:
                await conv_manager.close()
                await manager.close(keep_browser_open=True)
        ...
        try:
            ...
            try:
                ...
                for conv_id, success in results.items():
                    if success:
                        console.print(f"  [green]✓[/green] {conv_id} → {space_id}")
                    else:
                        console.print(f"  [red]✗[/red] {conv_id} → {space_id}")
            finally:
                await conv_manager.close()
                await manager.close(keep_browser_open=True)
    ...
    async def manage_interactive(self):
        """Interactive conversation manager - goes directly to conversation selection"""
        ...
        # Load spaces and conversations at startup
        console.print("[dim]Loading spaces...[/dim]")
        current_spaces = await conv_manager.list_spaces(limit=50)
        if current_spaces:
            console.print(f"[green]✓ Loaded {len(current_spaces)} spaces[/green]")
            if hasattr(conv_manager, 'log') and 'cached' in getattr(conv_manager, 'log', ''):
                console.print("[yellow]Warning: Using cached spaces. API failed.[/yellow]")
        else:
            console.print("[yellow]No spaces found[/yellow]")
            console.print("[yellow]Warning: No cached spaces available.[/yellow]")

        show_unspaced_only = True  # Default: show only conversations not in spaces
        current_offset = 0
        batch_size = 50
        loaded_conversation_ids = set()
        all_conversations = []
        
        while True:
            try:
                # Load conversations each iteration (to get latest state)
                filter_text = " (unspaced only)" if show_unspaced_only else " (all)"
                if current_offset == 0:
                    console.print(f"[dim]Loading conversations{filter_text}...[/dim]")
                    loaded_conversation_ids = set()
                    all_conversations = []
                else:
                    console.print(f"[dim]Loading more conversations (offset: {current_offset}){filter_text}...[/dim]")
                
                new_conversations = await conv_manager.list_conversations(limit=batch_size, offset=current_offset, unspaced_only=show_unspaced_only)
                
                # Filter out duplicates
                unique_new = [c for c in new_conversations if c.id not in loaded_conversation_ids]
                for c in unique_new:
                    loaded_conversation_ids.add(c.id)
                all_conversations.extend(unique_new)
                
                current_conversations = all_conversations

                if not current_conversations:
                    console.print("[yellow]No conversations found[/yellow]")
                    if show_unspaced_only:
                        console.print("[dim]Try showing all conversations instead[/dim]")
                        show_unspaced_only = False
                        continue
                    break

                indices = await self.interactive_select(
                    f"Conversations{filter_text}",
                ...
                action_result = await radiolist_dialog(
                    title="Action",
                    text=f"Choose an action ({len(current_conversations)} conversations loaded):",
                    values=[
                        ("open", "Open in browser"),
                        ("move", "Move to space..."),
                        ("delete", "Delete conversations"),
                        ("load_more", f"Load more (+{batch_size})"),
                        ("toggle", "Show all / Unspaced only"),
                ...
                if action_result == "open":
                    ...
                    # Continue to refresh conversations
                    ...
                    if space_indices:
                        ...
                        for i in indices:
                            ...
                            if success:
                                console.print(f"[green]✓ Moved: {current_conversations[i].title[:40]}[/green]")
                            else:
                                console.print(f"[red]✗ Failed: {current_conversations[i].title[:40]}[/red]")
                    ...
                    for i in indices:
                        ...
                        if success:
                            console.print(f"[green]✓ Deleted: {current_conversations[i].title[:40]}[/green]")
                            # Remove from loaded set so it doesn't reappear
                            loaded_conversation_ids.discard(current_conversations[i].id)
                        else:
                            console.print(f"[red]✗ Failed: {current_conversations[i].title[:40]}[/red]")
                    # Reset offset to refresh from beginning after delete
                    current_offset = 0
                    # Continue to refresh conversations automatically after delete
                elif action_result == "load_more":
                    current_offset += batch_size
                    # Continue to load more conversations
                elif action_result == "refresh":
                    current_offset = 0
                    # Just continue to reload
                elif action_result == "toggle":
                    show_unspaced_only = not show_unspaced_only
                    console.print(f"[dim]Switched to {'unspaced only' if show_unspaced_only else 'all conversations'}[/dim]")
                    # Reset offset when switching filter modes
                    current_offset = 0
                    # Continue to refresh with new filter
                    ...
                    if len(parts) > 1:
                        ...
                        for conv_id in ids_to_delete:
                            ...
                            if success:
                                ...
                                console.print(f"[red]✗ Failed: {conv_id}[/red]")
                        current_conversations = []
                    ...
                    if len(parts) > 2:
                        ...
                        for conv_id in conv_ids:
                            ...
                            if results.get(conv_id):
                                console.print(f"[green]✓ Moved: {conv_id} → {space_id}[/green]")
                            else:
                                console.print(f"[red]✗ Failed: {conv_id}[/red]")
                        current_conversations = []
