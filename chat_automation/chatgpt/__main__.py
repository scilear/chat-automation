...
from chat_automation.cli_common import (
    ...
    parse_persona,
    load_persona,
    list_personas as list_personas_func,
)
...
class ChatGPTCLI:
    ...
    async def export_conversation_markdown(
        ...
        try:
                    ...
                    try:
                        ...
                        if dump_dir and is_interesting:
                            ...
                            if len(body.encode("utf-8")) > max_bytes:
                                body = body.encode("utf-8")[:max_bytes].decode(
                                    "utf-8", errors="ignore"
                                )
                            safe_name = (
                                url.replace("https://", "")
            ...
            if force_reload or current_url != chat_url:
                ...
                console.print(
                    "[dim]Already on target conversation, skipping reload[/dim]"
                )
            ...
            if use_network:
                try:
                    ...
                    if raw_text is None:
                        if log_network:
                            console.print(
                                "[dim]No response body captured; trying direct fetch[/dim]"
                            )
                    ...
                    if isinstance(data, dict) and data.get("detail"):
                        if log_network:
                            console.print(
                                "[dim]First fetch failed; retrying after short wait[/dim]"
                            )
                        await asyncio.sleep(3)
                        raw_text = await _fetch_via_page()
                    ...
                    if data:
                        ...
                        if not messages:
                            raise RuntimeError(
                                "No messages found in backend API response"
                            )
                    ...
                    if log_network:
                        console.print(
                            f"[dim]Network error body: {raw_text[:500]}[/dim]"
                        )
            ...
            if not messages_loaded:
                console.print(
                    "[yellow]Warning:[/yellow] No messages detected yet; export may be empty"
                )
    ...
    async def chat_message(
        self, message: str, session_id: str = None, output_file: str = None
    ):
        """Send a chat message"""
        ...
        if persona_name:
            ...
            if persona_content:
                ...
                console.print(
                    f"[yellow][Warning: Persona '{persona_name}' not found][/yellow]\n"
                )
        ...
        try:
            ...
            if self.chat._current_conversation.url:
                ...
                console.print(
                    f"[dim]Session ID: {self.chat._current_conversation.id}[/dim]"
                )
                console.print(f"\n[dim]Continue with:[/dim]")
                console.print(
                    f'[dim]  ./chatgpt continue {self.chat._current_conversation.id} "your next message"[/dim]'
                )
    ...
    async def chat_voice_file(
        ...
        """Transcribe audio file via ChatGPT web session and send as message."""
        if not os.path.exists(filepath):
            console.print(f"[red]Error: File not found: {filepath}[/red]")
            return
        ...
        try:
            ...
            console.print(f"[dim]Transcribing audio: {filepath}[/dim]")
            if message:
                console.print(f"[dim]Prefix: {message}[/dim]\n")

            response = await self.chat.send_voice_file(filepath, message)
    ...
    async def transcribe_voice_file(self, filepath: str, output_file: str = None):
        """Transcribe audio file via ChatGPT web session without sending it."""
        if not os.path.exists(filepath):
            console.print(f"[red]Error: File not found: {filepath}[/red]")
            return
        ...
        try:
            console.print(f"[dim]Transcribing audio: {filepath}[/dim]")
            transcript = await self.chat.transcribe_voice_file(filepath)

            if transcript.startswith("Error:"):
                console.print(f"[red]{transcript}[/red]")
                return

            if output_file:
                with open(output_file, "w") as f:
                    f.write(transcript)
                console.print(f"[dim]Transcript written to: {output_file}[/dim]")
            else:
                console.print()
    ...
    async def show_history(self, session_id: str = None):
        ...
        if session_id:
            ...
            for msg in data.get("messages", []):
                ...
                console.print(
                    f"[bold {role_style}]{role}:[/bold {role_style}] {content}\n"
                )
        else:
    ...
    async def interactive(self, session_id: str = None):
        ...
        console.print(
            "[dim]Commands: /exit /new /save /load <id> /voice /voicefile <path> /transcribe <path> /help[/dim]"
        )
            ...
            while True:
                try:
                    ...
                    if line.strip() == "/exit":
                        ...
                        console.print(
                            f"[dim]Saved: {self.chat._current_conversation.id}[/dim]\n"
                        )
                        continue
                        ...
                        if voice.start_recording():
                            ...
                            if transcript:
                                ...
                                console.print(
                                    f"[dim]Transcribed{time_str}: {transcript}[/dim]"
                                )
...
[dim]Commands:
  ...
  /voicefile   Send audio file via web transcribe
  /transcribe  Transcribe audio file only
  /help        Show this help
...
""")
                    ...
                    elif line.strip().startswith("/voicefile "):
                        audio_path = line.strip().split(None, 1)[1]
                        console.print(
                            f"[dim]Transcribing audio file: {audio_path}[/dim]"
                        )
                        transcript_response = await self.chat.send_voice_file(
                            audio_path
                        )
                        if transcript_response.startswith("Error:"):
                            console.print(f"[red]{transcript_response}[/red]")
                        else:
                            console.print()
                            console.print("[bold green]ChatGPT:[/bold green]")
                            console.print(Markdown(transcript_response))
                            console.print()
                            await self.chat.export_conversation(
                                str(
                                    SAVE_DIR
                                    / f"{self.chat._current_conversation.id}.json"
                                )
                            )
                        continue

                    elif line.strip().startswith("/transcribe "):
                        audio_path = line.strip().split(None, 1)[1]
                        console.print(
                            f"[dim]Transcribing audio file: {audio_path}[/dim]"
                        )
                        transcript = await self.chat.transcribe_voice_file(audio_path)
                        if transcript.startswith("Error:"):
                            console.print(f"[red]{transcript}[/red]")
                        else:
                            console.print()
                        ...
                        if persona_name:
                            ...
                            if not persona_content:
                                console.print(
                                    f"[yellow]Persona '{persona_name}' not found[/yellow]\n"
                                )
                        ...
                        if not response or response.startswith("Error"):
                            console.print(
                                f"[red]Failed to get response: {response}[/red]"
                            )
                            continue
...
Examples:
  ...
  chatgpt chat --voice-file note.m4a         # Transcribe audio and send
  chatgpt transcribe note.m4a                # Transcribe audio only
...
Flags:
  ...
  -q, --quiet      Disable spinner animation
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Disable spinner animation"
    )
    ...
    chat_parser.add_argument("--voice-file", help="Audio file to transcribe and send")
    ...
    chat_parser.add_argument(
        "--output", "-o", help="Write response to file (markdown format)"
    )
    ...
    transcribe_parser = subparsers.add_parser(
        "transcribe", help="Transcribe an audio file only"
    )
    transcribe_parser.add_argument("file", help="Audio file to transcribe")
    transcribe_parser.add_argument("--output", "-o", help="Write transcript to file")

    # Export conversation (new)
    export_parser = subparsers.add_parser(
        "export", help="Export conversation as markdown"
    )
    export_parser.add_argument(
        "conversation_id", help="Conversation URL ID (after /c/)"
    )
    export_parser.add_argument(
        "--group",
        "-g",
        help="Group/project ID for /g/<group_id>/c/<conversation_id> URLs",
    )
    export_parser.add_argument(
        "--force-reload",
        action="store_true",
        help="Force reload even if already on target URL",
    )
    export_parser.add_argument(
        "--log-network",
        action="store_true",
        help="Log matching network responses during export",
    )
    export_parser.add_argument(
        "--no-network", action="store_true", help="Disable backend API export path"
    )
    export_parser.add_argument(
        "--dump-network-dir", help="Write matching response bodies to this folder"
    )
    export_parser.add_argument(
        "--output",
        "-o",
        help="Write export to file (default: chatgpt_conversation_<id>.md)",
    )
    ...
    if args.command == "login":
        ...
        # Determine message content: from prompt file if provided, else from message arg
        if args.prompt_file:
        ...
        if args.file:
            asyncio.run(
                cli.chat_file(args.file, message_content, args.session, output_file)
            )
        elif args.voice_file:
            asyncio.run(
                cli.chat_voice_file(
                    args.voice_file, message_content, args.session, output_file
                )
            )
        else:
            asyncio.run(cli.chat_message(message_content, args.session, output_file))
        ...
        output_file = getattr(args, "output", None)
        asyncio.run(cli.transcribe_voice_file(args.file, output_file))
